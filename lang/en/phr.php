<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Patient History Sheet Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines contain the default error messages used by
    | the Patient History Sheet class.
    |
    */
    'gender' => [
        '1'=>'Male',
        '2'=>'Female',
        '3'=>'Others',
    ],
    'category' => [
        '1'=>'General',
        '2'=>'OBC',
        '3'=>'SC',
        '4'=>'ST',
        '5'=>'Others',
    ],
    'age_group' => [
        '1'=>'Child',
        '2'=>'Young',
        '3'=>'Old',
    ],
    'occupation' => [
        '1'=>'Private',
        '2'=>'Businessman',
        '3'=>'Goverment',
        '3'=>'Semi Private',
        '4'=>'Others',
    ],
    'skin' => [
        '1'=>'Touch-Shita',
        '2'=>'Ushna',
        '3'=>'Ushna',
        '4'=>'Swedala',
        '5'=>'Ruksha',
        '6'=>'Shushka',
        '7'=>'Shitala',
        '8'=>'kathinya',
        '9'=>'Anya',
    ],
    'nadi' => [
        '1'=>'Manda',
        '2'=>'Tivra',
        '3'=>'Sama',
    ],
    'place' => [
        '1'=>'Antah',
        '2'=>'Madhya',
        '3'=>'Bahya',
        '4'=>'Peedanavastha',
        '5'=>'Anya',
    ],
    'nails' => [
        '1'=>'Kathinya',
        '2'=>'Mridu',
        '3'=>'Rekhankita',
        '4'=>'Kurchakara',
        '5'=>'Anya',
    ],
    'anguli_sandhi' => [
        '1'=>'Kathinyata',
        '2'=>'Samanya',
        '3'=>'Ubharata',
        '4'=>'Vakrata',
        '5'=>'Abha',
        '6'=>'Anya',
    ],
     'netra' => [
        '1'=>'Shvetabhaga',
        '2'=>'Rakta',
        '3'=>'Shweta',
        '4'=>'Dhusarita',
        '5'=>'Rakta Keshika',
        '6'=>'Peeta',
        '7'=>'Neela',
        '8'=>'Anya',


    ],
    'adhovartma' => [
        '1'=>'Shofa',
        '2'=>'Syahata',
        '3'=>'Samanya',
        '4'=>'Anya',
    ],
    'hastatala' => [
        '1'=>'Shitala',
        '2'=>'Shushka',
        '3'=>'Ruksha',
        '4'=>'Kathora',
        '5'=>'Mridu',
        '6'=>'Ushna',
        '7'=>'Swedala',
        '8'=>'Anya',


    ],
    'jihwa' => [
        '1'=>'Rukshta',
        '2'=>'Shushkata',
        '3'=>'Sphutita',
        '4'=>'Aavarta',
        '5'=>'Krishna',
        '6'=>'Aakritiwan',
        '7'=>'Konakar',
        '8'=>'Kurchakar',
        '9'=>'Srawi',
        '10'=>'Anya',


    ],



    'aakriti' => [
        '1'=>'Pandur',



        '2'=>'Aabhawan',
        '3'=>'Shotha',
        '4'=>'Syahata',
        '5'=>'Neelima',
        '6'=>'Vakarta',
        '7'=>'Arati',
        '8'=>'Aartta',
        '9'=>'Bhavuka',
        '10'=>'Anya',



    ],

    'shabda' => [
        '1'=>'Samanya',



        '2'=>'Kathora',
        '3'=>'Aartta',
        '4'=>'Sphuta',
        '5'=>'Manda',
        '6'=>'Abhava',
        '7'=>'Gambhir',
        '8'=>'Anya',




    ],

    'koshtha' => [
        '1'=>'Krura',
        '2'=>'Mridu',
        '3'=>'Madhya',
        '4'=>'Sama',

    ],

    'agni' => [
        '1'=>'Samanya',
        '2'=>'Manda',
        '3'=>'Tivra',
        '4'=>'Sama',

    ],

    'mala_pravritti' => [
        '1'=>'Samanya',
        '2'=>'Adhika',
        '3'=>'Sankya',
        '4'=>'Ushnata',
        '5'=>'Shitala',
        '6'=>'Lalasravi',
        '7'=>'Rakta',
        '8'=>'Dhusarita',
        '9'=>'Durgandhita',
        '10'=>'Anya',



    ],

    'mutra_pravritti' => [
        '1'=>'Samanya',
        '2'=>'Alpa',
        '3'=>'Adhika',
        '4'=>'Sankya',
        '5'=>'Ushnata',
        '6'=>'Sheetala',
        '7'=>'Lalasrawi',
        '8'=>'Raktayukta',
        '9'=>'Peeta',
        '10'=>'Dhusarita',
        '11'=>'Durgandhita',
        '12'=>'Anya',

    ],

     'vyavay_pravritti' => [
        '1'=>'Saptaha',
        '2'=>'Pakshika',
        '3'=>'Masika',
        '4'=>'Anya',

    ],

     'shukrakshana_pravritti' => [
        '1'=>'Alpa',
        '2'=>'Adhika',
        '3'=>'Samanya',
        '4'=>'Gandha',
        '5'=>'Anya',

    ],




    'aartava_pravratti_kala' => [
        '1'=>'Divasaantara',
        '2'=>'Purva',
        '3'=>'Pashchat',
        '4'=>'Alpa',
        '5'=>'Ati',
        '6'=>'Varna',
        '7'=>'Krishna',
        '8'=>'Rakta',
        '9'=>'Dhusarita',
        '10'=>'Durgandhita',
        '11'=>'Anya',


    ],
    'dehoshma' => [
        '1'=>'Tapaman',


    ],

     'raktachapa' => [
        '1'=>'Uchchatama',
        '2'=>'Nyuntama',
        '3'=>'Madhyantara',

    ],




    'hrid_gati' => [
        '1'=>'Samanya',
        '2'=>'Ati',
        '3'=>'Manda',
        '4'=>'Asthir',
        '5'=>'Samanya',
        '6'=>'Aakar',
        '7'=>'Anya',

    ],
    'shvasagati' => [
        '1'=>'Manda',
        '2'=>'Ati',
        '3'=>'Vishama',
        '4'=>'Samanya',

    ],

    'parkriti_parikshana' => [
        '1'=>'Janmajata prakriti',
        '2'=>'Roga prakriti',

    ],

    'examination_by_physician' => [
        '1'=>'Prabhavita Sansthana',
        '2'=>'Vata Pitta Kapha',
        '3'=>'Roga Nama (Purva Prarambhika Nidana)',
        '4'=>'Tatkalika Nidana',

    ],

    'marital_status' => [
        '1'=>'Married',
        '2'=>'Unmarried',


    ],
    'region_of_patient' => [
        '1'=>'Jamgala',
        '2'=>'Anupa',
        '3'=>'Sadharana',



    ],

    'title' => [
        '1'=>'Mr.',
        '2'=>'Mrs.',
        '3'=>'Miss.',
        '4'=>'Ms.',

    ],

    'yogas' => [
        '1'=>'CHURNA YOGAS',
        '2'=>'RASA YOGAS',
        '3'=>'VATI YOGAS',
        '4'=>'TAILA – GHRITA YOGAS',
        '5'=>'ASAVA/ARISHTA YOGAS',

    ],

    'titlename' => [
        '1'=>'Mr.',
        '2'=>'Mrs.',
        '3'=>'Miss.',
        '4'=>'Ms.',
        ],

    'user_type' => [
        '1'=>'Admin',
        '2'=>'Guru',
        '3'=>'Shishya',
        ],

    'user_type_without_admin' => [
        '2'=>'Guru',
        '3'=>'Shishya',
        ],

    'user_type_model' => [
        '0'=>'All',
        '4'=>'Super Admin',
        '1'=>'Admin',
        '2'=>'Guru',
        '3'=>'Shishya',
        ],

    'patients_type' => [
        '1'=>'In-Patient',
        '2'=>'OPD-Patient',
        ],

    'guru_type' => [
        'Individual'=>'Individual',
        'Institutional'=>'Institutional',
        ],

    'shishya_type' => [
        'Pharmacy'=>'Pharmacy',
        'Non Pharmacy'=>'Non Pharmacy',
        ],

    'course' => [
        '1'=>'php',
        '2'=>'react',
        '3'=>'jquery',
        '4'=>'javascript',
        ],

    'education_course' => [
        '1'=>'php',
        '2'=>'BMS',
        '3'=>'MB',
        '4'=>'MS',
        '5'=>'MCA',
        '6'=>'BCA',
        ],

        
'post_course' => [
        '1'=>'php',
        '2'=>'react',
        '3'=>'jquery',
        '4'=>'vue',
        '5'=>'javascript',
        '6'=>'angular',
        ],

'working_days' => [
        '0'=>'Monday',
        '1'=>'Tuesday',
        '2'=>'Wednesday',
        '3'=>'Thursday',
        '4'=>'Friday',
        '5'=>'Saturday',
        '6'=>'Sunday',
        ],

'clinic_evening_timing' => [
        '0'=>'4pm to 5pm',
        '1'=>'5pm to 6pm',
        '2'=>'6pm to 7pm',
        ],

'clinic_morning_timing' => [
        '0'=>'7am to 8am',
        '1'=>'8am to 9am',
        '2'=>'9am to 10am',
       ],

'attendance_evening_timing' => [
        '0'=>'4pm to 5pm',
        '1'=>'5pm to 6pm',
        '2'=>'6pm to 7pm',
        ],

'attendance_morning_timing' => [
        '0'=>'7am to 8am',
        '1'=>'8am to 9am',
        '2'=>'9am to 10am',
       ],

'banks' => [
        '1' => 'Allahabad Bank',
        '2' => 'Andhra Bank',
        '3' => 'Axis Bank',
        '4' => 'Bank of Bahrain and Kuwait',
        '5' => 'Bank of Baroda - Corporate Banking',
        '6' => 'Bank of Baroda - Retail Banking',
        '7' => 'Bank of India',
        '8' => 'Bank of Maharashtra',
        '9' => 'Canara Bank',
        '10' => 'Central Bank of India',
        '11' => 'City Union Bank',
        '12' => 'Corporation Bank',
        '13' => 'Deutsche Bank',
        '14' => 'Development Credit Bank',
        '15' => 'Dhanlaxmi Bank',
        '16' => 'Federal Bank',
        '17' => 'ICICI Bank',
        '18' => 'IDBI Bank',
        '19' => 'Indian Bank',
        '20' => 'Indian Overseas Bank',
        '21' => 'IndusInd Bank',
        '22' => 'ING Vysya Bank',
        '23' => 'Jammu and Kashmir Bank',
        '24' => 'Karnataka Bank Ltd',
        '25' => 'Karur Vysya Bank',
        '26' => 'Kotak Bank',
        '27' => 'Laxmi Vilas Bank',
        '28' => 'Oriental Bank of Commerce',
        '29' => 'Punjab National Bank - Corporate Banking',
        '30' => 'Punjab National Bank - Retail Banking',
        '31' => 'Punjab & Sind Bank',
        '32' => 'Shamrao Vitthal Co-operative Bank',
        '33' => 'South Indian Bank',
        '34' => 'State Bank of Bikaner & Jaipur',
        '35' => 'State Bank of Hyderabad',
        '36' => 'State Bank of India',
        '37' => 'State Bank of Mysore',
        '38' => 'State Bank of Patiala',
        '39' => 'State Bank of Travancore',
        '40' => 'Syndicate Bank',
        '41' => 'Tamilnad Mercantile Bank Ltd.',
        '42' => 'UCO Bank',
        '43' => 'Union Bank of India',
        '44' => 'United Bank of India',
        '45' => 'Vijaya Bank',
        '46' => 'Yes Bank Ltd',
    ],

    'soft_drink' => [ 
        '1' => 'Milk',
        '2' => 'Tea',
        '3' => 'Coffee',
        '4' => 'Neera',
        '5' => 'Butter-milk',
        '6' => 'Lassi',
        '7' => 'Toned-Milk',
        '8' => 'SoyaMilk',
        '9' => 'Soup of Moong Dal',
        '10' => 'Bala svarasa Panaka',
        '11' => 'Guduchi swaras Panak',
        '12' => 'Durvasvarasa Panak',
        '13' => 'Vijayasar Shrit Sheet Jal/Cold Water',
        '14' => 'Nimb Patra Swaras',
        '15' => 'Soup',
        '16' => 'Mamsaras',
        '17' => 'Butter',
        '18' => 'Mishri',
       ],
    'prataraasha' => [ 
        '1' => 'Milk',
        '2' => 'Tea',
        '3' => 'Coffee',
        '4' => 'Makhanna',
        '5' => 'Toast',
        '6' => 'Bread',
        '7' => 'Jam',
        '8' => 'Poha',
        '9' => 'Chiwda',
        '10' => 'Lai',
        '11' => 'Upma',
        '12' => 'Halwa',
        '13' => 'Dalia',
        '14' => 'Khichadi',
        '15' => 'Butter',
        '16' => 'Egg',
        '17' => 'Curd',
        '18' => 'Jalebi',
        '19' => 'sweet fried- Salted fried- Padarth',
        '20' => 'Sabudana',
        '21' => 'Ankurit Dhanya (Sprouting pulse)-(Monga',
        '22' => 'Motha',
        '23' => 'Chana wheat) Bala Snehana',
        '24' => 'Madhuyashti Snehan',
        '25' => 'Nimba snehana',
       ],
    'pulses' => [ 
        '1' => 'Moonga',
        '2' => 'Urada',
        '3' => 'Masur',
        '4' => 'Tuar',
        '5' => 'Chana',
        '6' => 'Matar',
        '7' => 'Semphali',
        '8' => 'Chawala',
        '9' => 'Falli',
        '10' => 'Ganwar',
        '11' => 'Falli',
        '12' => 'Dahi',
        '13' => 'Rayata',
        '14' => 'Paneer',
        '15' => 'Kari',
        '16' => 'Chhachh',
        '17' => 'Soup',
        '18' => 'Tomato',
        '19' => 'Chatani',
        '20' => 'Shakarkand',
        '21' => 'Aalu',
        '22' => 'Arbi',
        '23' => 'Jamikanda',
        '24' => 'Cauliflower',
        '25' => 'Cabbage',
        '26' => 'Pyaj (Onion)',
        '27' => 'Lahsun',
        '28' => 'Adrak',
        '29' => 'Aanwala',
        '30' => 'Katahal',
        '31' => 'Mushroom',
       ],
    'pulpy_vegetables' => [ 
        '1' => 'Lauki',
        '2' => 'Turai',
        '3' => 'Ghiya',
        '4' => 'Tinda',
        '5' => 'Parwal',
        '6' => 'Sahjan',
        '7' => 'Papita',
        '8' => 'Bhindi',
        '9' => 'Salajam',
        '10' => 'Gajar',
        '11' => 'Muli',
        '12' => 'Mli leaf',
        '13' => 'Bathua',
        '14' => 'Dhaniya',
        '15' => 'Pyaaj leaf',
        '16' => 'Harimirch',
        '17' => 'Meetha Neem',
        '18' => 'Imli tender leaves',
        '19' => 'Pickle (Aam Nibu Mircha) vinegar',
       ],
    'oil_tail' => [ 
        '1' => 'Sesamum',
        '2' => 'ground nut',
        '3' => 'Alsi',
        '4' => 'Soyabean',
        '5' => 'mustard',
        '6' => 'sunflower',
        '7' => 'Pamolien',
        '8' => 'Safola',
        '9' => 'Flora',
        '10' => 'ghee of cow',
        '11' => 'buffalo',
        '12' => 'camel',
        '13' => 'she goat',
       ],
    'afternoon_fruit' => [ 
        '1' => 'Papita',
        '2' => 'sapota',
        '3' => 'apple',
        '4' => 'Nasapati',
        '5' => 'pineapple',
        '6' => 'Litchi',
        '7' => 'pomegranate',
        '8' => 'orange',
        '9' => 'Mosambi',
        '10' => 'Anjeer',
        '11' => 'Angoor',
        '12' => 'Shahatuta',
        '13' => 'Falsa',
        '14' => 'Kela',
        '15' => 'Kakari',
        '16' => 'Khira',
        '17' => 'Tarbuja',
        '18' => 'Jamun',
        '19' => 'Aam',
        '20' => 'Kharbuja',
        '21' => 'Singara',
        '22' => 'Sitaphala',
        '23' => 'Nariyalapani',
        '24' => 'Narikela',
       ],
    'evening_meals' => [ 
      
       ],
    'bed_time' => [ 
        '1' => 'Garam Dhudh',
        '2' => 'medicine (if any) with ghee',
        '3' => 'Others',
       ],
   
    'madhyahan_bhojana' => [ 
        '1' => 'Roti',
        '2' => 'Parantha',
        '3' => 'Tanduri',
        '4' => 'Roti (Joi, Gehun, Jwara)',
        '5' => 'Upma',
        '6' => 'Bread',
        '7' => 'Toast',
        '8' => 'Uttapam',
        '9' => 'Masala Doda',
        '10' => 'Rice/Chawala',
        '11' => 'Khichadi',
        '12' => 'Daliya',
        '13' => 'Sabudana',
        '14' => 'Suji',
       ],
   
];
